    ...
    try {
      const emailData = {
        ...
        orderItems: orderItems.map((item) => ({
          product: state.items.find(
            (cartItem) => cartItem.product.id === item.product_id
          )?.product,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasSupabaseConfig) {
      toast({
        title: "Demo Mode",
        description:
          "Order placed successfully! (Demo mode - no real payment processed)",
      });
      clearCart();
      router.push("/");
      return;
    ...
    try {
      const orderData = {
        ...
        payment_status:
          formData.paymentMethod === "cash_on_delivery" ? "pending" : "paid",
      ...
      toast({
        ...
        description: `Your order #${order.id.slice(
          0,
          8
        )} has been placed. Admin has been notified.`,
      });
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    <Label htmlFor="alternativePhone">
                      Alternative Phone Number (11 digits)
                    </Label>
                    <Input
                      id="alternativePhone"
                      type="tel"
                      ...
                      onChange={(e) =>
                        handlePhoneChange(e.target.value, "alternativePhone")
                      }
                      maxLength={11}
                    />
                    {formData.alternativePhone && (
                      <p className="text-xs text-gray-500 mt-1">
                        {formData.alternativePhone.length}/11 digits
                      </p>
                    )}
                  </div>
                </CardContent>
                    ...
                    <div>
                        ...
                        <SelectContent>
                          {governorates.map((gov) => (
                            <SelectItem key={gov.id} value={gov.governorate}>
                              {gov.governorate} -{" "}
                              {gov.delivery_price.toFixed(2)} EGP
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      {formData.governorate && (
                        <p className="text-xs text-green-600 mt-1">
                          Delivery: {deliveryPrice.toFixed(2)} EGP
                        </p>
                      )}
                    </div>
                  </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="border-t pt-4 space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    <span>
                      Delivery ({formData.governorate || "Select governorate"})
                    </span>
